...
#' Title
#'
#' @param model_output liste and SWM class object. The output of SWB_model.
#' @param what The name of the variable to display, among those contained in model_output$daily_aggregated_variables
#' @param type The type of graph (corresponds to a geom_ in ggplot's namespace)
...
#' @param device if save_graph = TRUE, the device used
...
# General -----------------------------------------------------------------
  ...
  if(length(what) > 1){
    if(all(what%in%c("REW_global","drainage","restTr"))){
      plot <- .facet_SWM_outputs(model_stack = model_stack, what = what, type = type, ...)
    }
    else stop("argument what must be one or several of: 'REW_global', 'drainage','resTr'")
  }
  else if(!what%in%c("REW_global","drainage","restTr")){
  ...
  else plot <- .display_single_stacked_variable(model_stack = model_stack, what = what, type = type, ...)
